<?php
	...
	$path_data = fetch("SELECT * FROM files WHERE idx = '{$path}';");
	$parentURL = "";
	if($path_data){
		$parentURL = HOME_URL."/file?path={$path_data->parent}";
	}
 ?>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<button type="button" onclick="location.href='<?php echo HOEM_URL ?>/file/deleteFile/<?php echo $data->idx ?>'; return false;">삭제</button>
			</td>
		</tr>
		<?php endforeach ?>
...
<form action="" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="action" value="<?php echo $path ?>">
	<label> 파일 선택 : 
